...
class UserConfig(BaseModel):
    """Modelo base de validação de dados para a tabela 'usuario'."""
    ...
    # Campos base do Usuário (tabela 'usuario')
    ...
    lv_acesso: Optional[NivelAcessoEnum] = Field(None, title='Nível de Acesso', description='Supremo, Colaborador, Instrutor ou Aluno.') # Deixado como Optional, pois as classes filhas definirão a função.

    # Aplicações de Conta
    tipo_email: TipoEmailEnum = Field(..., title='Tipo de E-mail', description='PESSOAL ou COMERCIAL.')
    email_user: str = Field(..., max_length=255, title='E-mail do Usuário') # Poderia usar EmailStr do Pydantic para validação mais estrita
    ...
    # Relacionamento com Contato (Lista de contatos)
