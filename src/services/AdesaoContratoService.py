...
class AdesaoContratoService:
    ...
    # def delete_adesao_and_contract(self, adesao_id: int) -> None:
    #     """Deleta o Contrato e a Adesão associada de forma atômica, delegando aos Models."""
    #     try:
    #         is_contract_deleted = self.contrato_model.delete_contrato_by_adesao_id(adesao_id)
    #         is_adesao_deleted = self.adesao_model.delete_adesao_by_id(adesao_id)
            
    #         if not is_adesao_deleted and not is_contract_deleted:
    #             self.session.rollback()
    #             raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, 
    #                                 detail=f"Adesão com ID {adesao_id} não encontrada para exclusão.")

    #         # 3. COMMIT ÚNICO para garantir a atomicidade (se os Model methods não fizeram o commit, como é o ideal)
    #         # Nota: Revise seus Model methods de delete para garantir que eles *não* façam o commit.
    #         self.session.commit()
    ...
    #                             detail="Falha ao concluir a transação de exclusão devido a um erro de banco de dados.")


    # ## 2. Método PATCH Refatorado para usar Model Selects
    # def update_adesao_and_contract(self, adesao_id: int, data_update: AdesaoPlanoUpdate) -> Dict[str, Any]:
    #     """Atualiza a Adesão e propaga a alteração para o Contrato associado, usando métodos select."""
        
    #     update_dict = data_update.model_dump(exclude_unset=True)
        
    #     # Usando select_adesao_by_id em vez de session.get
    #     adesao = self.adesao_model.select_adesao_by_id(adesao_id) 
        
    #     if not adesao:
    #         raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail=f"Adesão com ID {adesao_id} não encontrada.")
            
    #     contrato_db = self.contrato_model._get_contrato_by_adesao_id(adesao_id) # Usando select em ContratoModel

    #     if not update_dict:
    #         return {"adesao": adesao, "contrato": contrato_db} 

    #     try:
    #         # 1. Atualiza a Adesão (Seu Model já faz o commit internamente)
    #         updated_adesao = self.adesao_model.update_adesao_plano(adesao_id, update_dict)
            
    #         if not updated_adesao:
    #             # Se o update_adesao_plano retornar None, mas a adesao foi encontrada acima, algo deu errado.
    #             raise Exception("Falha interna ao atualizar Adesão.")
                
    #         updated_contrato = contrato_db # Inicializa com o contrato encontrado
            
    #         # 2. Propaga a alteração para o Contrato, se campos relevantes mudaram
    #         if 'data_validade' in update_dict or 'fk_id_plano' in update_dict or 'fk_id_plano_personalizado' in update_dict:
                
    #             if contrato_db:
    #                 contrato_update_dict = {}
                    
    #                 if 'data_validade' in update_dict:
    #                     contrato_update_dict['data_termino'] = update_dict['data_validade']
                    
    #                 if 'fk_id_plano' in update_dict:
    #                     contrato_update_dict['fk_id_plano'] = update_dict['fk_id_plano']
    #                 if 'fk_id_plano_personalizado' in update_dict:
    #                     contrato_update_dict['fk_id_plano_personalizado'] = update_dict['fk_id_plano_personalizado']
                        
    #                 if contrato_update_dict:
    #                      # ⚠️ Usando o método update do ContratoModel
    #                      updated_contrato = self.contrato_model.update_contrato(contrato_db.id_contrato, contrato_update_dict)
                
    #         # O commit para Adesão e Contrato foi feito dentro dos respectivos Model methods (dado o seu design atual)
            
    #         return {"adesao": updated_adesao, "contrato": updated_contrato}

    #     except HTTPException:
    #         raise
    #     except Exception as e:
    #         self.session.rollback()
    #         logging.error(f"Erro ao atualizar adesão e contrato: {e}")
    #         raise HTTPException(status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail="Erro ao processar atualização.")